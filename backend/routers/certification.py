...
def create_certification(
    ...
    entry = Certification(user_id=current_user.id, **data.model_dump())
    db.add(entry)
    db.commit()
    db.refresh(entry)
    return entry
...
@router.put("/{entry_id}", response_model=CertificationResponse)
def update_certification(
    ...
    entry = db.query(Certification).filter(
        Certification.id == entry_id, Certification.user_id == current_user.id
    ).first()
    if not entry:
